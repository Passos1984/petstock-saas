...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(db_dir, 'petstock.db')
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['SECRET_KEY'] = 'petstock_master_2026_key'
...
def injetar_dados_globais():
    ...
    return dict(loja_logada=loja_atual, vendas_hoje="0,00", lucro_hoje="0,00", alertas_radar=0, cargo='Gerente', vendedor_atual='Dono/Gerente')
...
def login():
    if request.method == 'POST':
        ...
        flash('❌ Erro no login! Usuário ou senha incorretos.', 'danger')
    return render_template('login.html')
...
def home():
    ...
    return render_template('index.html', produtos=prods, proximo_sku=proximo_sku)
...
def editar_produto(id):
    ...
    if not prod or prod.loja_id != session['loja_id']: return redirect(url_for('home'))
...
def importar_produtos():
    ...
    try:
        ...
        flash(f'❌ Erro na importação: {str(e)}. Salve sua planilha como CSV (UTF-8).', 'danger')
...
# ==========================================
# --- 🛒 ROTAS DO NOVO PDV (FRENTE DE CAIXA) ---
# ==========================================
@app.route('/pdv')
def pdv():
    if 'loja_id' not in session: return redirect(url_for('login'))
...
def relatorios():
    ...
    return render_template('relatorios.html', vendas=vendas, total="%.2f" % total, lucro_total_formatado="%.2f" % (total - custo_total), pix="%.2f" % pix, pix_raw=pix, credito="%.2f" % credito, credito_raw=credito, debito="%.2f" % debito, debito_raw=debito, dinheiro="%.2f" % dinheiro, dinheiro_raw=dinheiro, fiado="%.2f" % fiado, fiado_raw=fiado, data_inicio=data_inicio, data_fim=data_fim, vendedor_filtro=vendedor_filtro, vendedores=[v[0] for v in vendedores_db if v[0]])
...
def admin_guilherme():
    ...
    hoje = data_brasil()
    if request.method == 'POST':
        db.session.add(Loja(nome_fantasia=request.form.get('nome_fantasia'), usuario=request.form.get('usuario').strip().lower(), senha=generate_password_hash('123456'), data_vencimento=datetime.strptime(request.form.get('data_vencimento'), '%Y-%m-%d').date()))
        db.session.commit()
        return redirect(url_for('admin_guilherme'))

    todas_lojas = Loja.query.all()
    mrr = sum(80.00 for loja in todas_lojas if loja.data_vencimento >= hoje)
    previsao_7_dias = sum(80.00 for loja in todas_lojas if hoje <= loja.data_vencimento <= hoje + timedelta(days=7))
...
def resetar_senha_loja(id):
    if not session.get('ceo_logado'): return redirect(url_for('login_ceo'))
    Loja.query.get(id).senha = generate_password_hash('123456')
    db.session.commit()
    return redirect(url_for('admin_guilherme'))
...
# Cria as tabelas se elas não existirem (não apaga as antigas)
